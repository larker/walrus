<#if banners?? && (banners?size>0)>
	<#list banners as banner>
		<div class="bannerEditor-banner" id="banner_${boxId}_${banner.id}">
			...
			<a class="bannerDelete" title="Delete banner" href="#" onclick="if(confirm('确定删除?'))deleteBanner('${boxId}', '${banner.id}');return false;"><img src="${contextPath}/img/menu_handle.png" alt="Delete banner" title="Delete banner" style="width: 13px; height: 13px; margin: 0px;"/></a>
		</div>
	</#list>
</#if>